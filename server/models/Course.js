...
function createDefaultCourses(callback){
	if(!callback){
		callback = false
	}
	Course.find({}).exec(function(err, collection){
		if(collection.length === 0){
			...
			Course.create({title: 'Death March Coding for Fun and Profit', featured: true, published: new Date('7/1/2013'), tags: ['Coding', 'Misc']}, callback);
		}
	})
}
